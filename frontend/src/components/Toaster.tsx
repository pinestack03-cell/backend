...
const toastStyles = {
  ...
  info: { icon: <Info size={18} weight="fill" className="text-blue-500" /> },
};
    ...
    <div className="pointer-events-none fixed bottom-5 right-5 z-toast flex w-[340px] flex-col gap-2">
      {toasts.map((item) => (
        <div
          key={item.id}
          role="status"
          className="toast-enter pointer-events-auto flex items-start gap-2.5 rounded-lg border border-slate-200 bg-white px-3.5 py-3 shadow-lifted"
        >
